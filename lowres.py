...
def Juggernaut():
    ...
    while(True):
        ...
        ##If we are in a juggernaut battle
        elif(MI.locateCenterOnScreen('images/med_playerbubble.png', confidence = 0.70)):
            privateChat()
            # war_com()


            ##This statment continues to be run as long as we are alive or the battle is still going on 
            while(MI.locateCenterOnScreen('images/med_playerbubble.png', confidence = 0.70)):
                battleOver = True
                time.sleep(0.1)
                ...
                if MI.locateCenterOnScreen('images/med_strike.png', confidence = 0.9):
                    ...
                    # mouseMove()
                     #static granade
                    # MI.click(MI.locateCenterOnScreen('images/static_granade.png', confidence = 0.7))
                    # time.sleep(0.05)
            ...
            time.sleep(2)
            MI.click(598, 350)
            # _continue = MI.locateCenterOnScreen('images/continue.png', confidence = 0.7)
            # if(_continue):
            #     MI.click(_continue)
            if(battleOver):
